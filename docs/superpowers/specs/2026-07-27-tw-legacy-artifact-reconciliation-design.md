...
Every history envelope must match the outer artifact symbol, reproduce its full-envelope canonical SHA, contain a unique reconciled-artifact SHA, and describe a reconciliation no later than the current outer target. History is strictly ordered by unique reconciliation target. A lineage cannot contain both a direct `legacy_reconciliation` and `legacy_reconciliation_history`; artifact-SHA tampering, inner-record tampering, future, duplicate, unordered, cross-symbol, malformed, or mixed relations fail closed. This preserves the original audit evidence without making the current target identity internally impossible.
